...
use App\Services\DocumentGenerators\Traits\NatureFieldMapperTrait;
...
class AlienationMovablePropertyNotificationDocGenerator implements DocumentGeneratorInterface
{
    use DocumentFormatterTrait;
    use NatureFieldMapperTrait;

    public function generate(Notification $notification): string
    {
        $notification->load(['notifiable', 'notifiedPeople', 'addresses']);

        $templatePath = app(TemplateResolver::class)->resolve('notification_movable_alienation');
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'notificacao_m_');
    ...
    private function fillBasicData(TemplateProcessor $template, Notification $notification): void
    {
        ...
        $template->setValue('verb_notification', $peopleCount > 1 ? 'notificar-lhes' : 'notificar-lhe');

        if ($peopleCount > 1) {
            $interestShould = ($hasMale) ? 'os interessados deverão' : 'as interessadas deverão';
        } else {
            $interestShould = ($hasMale) ? 'o interessado deverá' : 'a interessada deverá';
        }
        $template->setValue('interest_should', $interestShould);

        $template->setValue('verb_aware', $peopleCount > 1 ? 'tomem' : 'tome');
    }
    ...
    private function fillSpecificData(TemplateProcessor $template, Notification $notification): void
    {
        ...
        $template->setValue('nature', 'Alienação Fiduciária de Bem Móvel');

        if (! $data) {
            $fields = ['credor', 'cnpj_number', 'office', 'contract_number', 'contract_date', 'total_amount_debt', 'emoluments_intimation', 'guarantee_movable_property_description', 'contract_registry_office', 'contract_registration_act', 'default_period', 'debt_position_date', 'contractual_clause'];
