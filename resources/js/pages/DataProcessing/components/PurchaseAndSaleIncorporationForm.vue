...
type PurchaseAndSaleIncorporationModel = App.Data.PurchaseAndSaleIncorporationData | null;

const props = defineProps<{
    modelValue: PurchaseAndSaleIncorporationModel;
    errors: any;
}>();

const emit = defineEmits<{
    (e: 'update:modelValue', value: PurchaseAndSaleIncorporationModel): void;
}>();

const defaultData: App.Data.PurchaseAndSaleIncorporationData = {
    id: 0,
    creditor: '',
    office: null,
    committed_property_registration: null,
    committed_property_identification: null,
...
const selectedRegistry = computed({
    ...
    set: (value) => {
        if (value === 'Outro') {
            const current = internalData.value.real_estate_registry_location;
...
const updateField = (fieldName: keyof App.Data.PurchaseAndSaleIncorporationData, value: string | number | boolean | null) => {
    const currentData = internalData.value;
    internalData.value = {
        ...currentData,
        [fieldName]: value,
    } as App.Data.PurchaseAndSaleIncorporationData;
};

const handleInput = (event: Event, fieldName: keyof App.Data.PurchaseAndSaleIncorporationData, type: 'text' | 'number') => {
...
<template>
    <div class="grid grid-cols-1 gap-5 p-6">
        ...
        <InputForm id="committed-property-registration" type="text" label="Matrícula do Imóvel Compromissado"
            placeholder="Digite a matrícula" v-maska="registrationMask"
            :model-value="internalData.committed_property_registration"
            @update:model-value="updateField('committed_property_registration', $event)"
            :error="errors?.notifiable?.committed_property_registration" />

        <InputForm id="committed-property-identification" type="text" label="Identificação do Imóvel Compromissado"
            placeholder="Digite o endereço completo do imóvel"
            :model-value="internalData.committed_property_identification"
            @update:model-value="updateField('committed_property_identification', $event)"
            :error="errors?.notifiable?.committed_property_identification" />

        <InputForm id="contract-registration-act" type="text" label="Ato da matrícula com registro do contrato"
            placeholder="Ex: R-01" v-maska="actMask" :model-value="internalData.contract_registration_act"
            @update:model-value="updateField('contract_registration_act', $event)"
            :error="errors?.notifiable?.contract_registration_act" />

        <InputForm id="emoluments-intimation" type="text" label="Emolumentos para intimação" placeholder="R$ 150,00"
        ...
        <InputForm id="total-amount-debt" type="text" label="Valor total da dívida" placeholder="R$ 10.000,00"
        ...
        <div class="flex flex-col gap-2">
            <label for="registry-select" class="block text-sm font-medium text-gray-700">
                Cartório onde o Imóvel Compromissado se Encontra
            </label>
            <div class="relative">
            ...
            <div v-if="selectedRegistry === 'Outro'" class="animate-fade-in-down">
                <InputForm id="real-estate-registry-location" type="text" label="Digite o nome do cartório (Manual)"
