...
import ButtonHistory from '@/pages/Notifications/components/ButtonHistory.vue';
import ButtonEdit from '@/pages/Notifications/components/ButtonEdit.vue';
...
const isReadOnly = computed(() => {
    return !!diligenceForActiveTab.value && !isEditing.value;
});
...
const enableEditing = () => {
    if (!diligenceForActiveTab.value) return;
    const currentDiligence = diligenceForActiveTab.value;

    form.defaults({
        visit_number: currentDiligence.visit_number,
        diligence_result_id: currentDiligence.diligence_result_id,
        observations: currentDiligence.observations ?? '',
        date: currentDiligence.date,
    });
...
const saveDiligence = () => {
    if (isEditing.value) {
        ...
        form.put(
            route('notifications.diligence.update', {
                notification: props.notification.protocol,
                address: props.address.id,
                diligence: diligenceForActiveTab.value.id,
            }),
            {
                preserveScroll: true,
                onSuccess: () => {
...
<template>
    ...
    <AppLayout :link-button="route('notifications.show', notification.protocol)" :has-parameter="true"
        text-button="Voltar" page-title="Fase de Notificação" method="get">
        ...
        <form @submit.prevent="saveDiligence"
            ...
            <div v-if="isReadOnly && diligenceForActiveTab" class="flex flex-col gap-6">
                <div>
                ...
                <div class="mt-4 flex flex-col sm:flex-row sm:justify-end sm:gap-x-4">
                    <ButtonHistory v-if="diligenceForActiveTab?.history?.length > 0" @click="showHistoryModal = true" />
        ...
        <HistoryModal :show="showHistoryModal" :diligence="diligenceForActiveTab" @close="showHistoryModal = false" />
    </AppLayout>
</template>